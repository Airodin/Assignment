...
public class DepartingTrainTest {
	...
	private static final String VALID_CLASSIFICATION = "6D";
	...
	public void testPassengerCarNumberOfSeats() throws TrainException{
		...
		assertTrue("Returns invald number of seats installed, expected " + PASSENGERCAR_SEAT_CAPACITY + " got " + departingTrain.numberOfSeats(),
				departingTrain.numberOfSeats() == PASSENGERCAR_SEAT_CAPACITY);
		//assert(departingTrain.numberOfSeats() == PASSENGERCAR_SEAT_CAPACITY);
	...
	public void testDepartingTraingCannotMoveIfTotalWeightExceedPullingPower()
		...
		DepartingTrain departingTrain = new DepartingTrain();
		departingTrain.addCarriage(new Locomotive(GROSS_WEIGHT, VALID_CLASSIFICATION));
		departingTrain.addCarriage(new PassengerCar(8 * GROSS_WEIGHT, PASSENGERCAR_SEAT_CAPACITY));
	 ...
	 * immediately after. Expect PassengerCar to be returned. 
	 * 
	 * @throws TrainException
	 */
	@Test
	...
	public void testTraverseTrainUsingNextCarriage()
		...
		assertFalse("first carriage is not locomotive", rollingStock instanceof Locomotive);
		carriageCount++;
		
		//Not sure what this is supposed to be doing, should we just check the nextCarriage() can go all the way through and back?
		...
		assertTrue("Next Carriage has not traverse the whole train", carriageCount == 3);
	}
